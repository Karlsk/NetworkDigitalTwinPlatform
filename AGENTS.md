...
| 错误码  | 模块     | 含义 |
|---------|----------|------|
| 400011  | Sync     | 不支持的同步动作 |
| 503011  | Sync     | Webhook 队列满 |
| 404021  | Snapshot | 快照未找到 |
| 500031  | Topology | 拓扑查询失败 |
| 404041  | Device   | Connector 未找到 |
| 404051  | Monitor  | 监控 Connector 未找到 |

#### 限流（Rate Limiting）

- 实现: `internal/api/middleware/ratelimit.go`，基于 `golang.org/x/time/rate` 令牌桶
- 默认: 100 req/s，burst=200
- 超限: 返回 429 + `{"code":429001,"message":"rate limit exceeded"}`
...
- 熔断时: 返回 503 + `{"code":503001,"message":"service unavailable, circuit breaker open"}`

#### Handler 编写规范

- Handler 不直接依赖具体 Service 实现，通过 HandlerDeps 接口注入
- 请求参数绑定失败 → 返回 400 + 错误码 400001（CodeBadRequest）
- Service 层返回 error → 返回 500 + 错误码 500001（CodeInternalError）
