...
**预估工时**: 18 人天（基础估算，不含缓冲；加 20% 缓冲后约 22 天）
...
⑤ consumer 消费 bus（异步，StartConsumer；消费侧单 goroutine 串行化）
   → graph.Upsert/DeleteByURIs/DeleteRelations   （default 物化投影）
   → graph 写成功后 MarkApplied(event_ids) 翻转 applied=true（非原子跨库，靠 MERGE 幂等+对账兜底）
```
...
// envelope 必含 event_id（下游用来 MarkApplied）。同一 SyncEvent 扇出后产生 N 条 bus 消息。
```
...
- **event_id 传递**：bus envelope 必含 `event_id`（来自 `topology_events.id`），作为 MarkApplied 的入参；现有 `events.SyncEvent` 结构需新增 `EventIDs []int64` 字段（V3-04 改造）

**投影一致性与发散检测**（Outbox 只保证投递不丢，**不保证 default 收敛**）：
- 维护「已应用事件 id 高水位」；周期性用 `PG 已落盘量 − 已应用量` 得投影积压指标并告警
- 或周期抽样比对 `default` 与 `Reconstruct(currentWatermark)`，发现发散触发 `RebuildDefault`（§4.6）
...
applyEvent(model, ev):
    ...
    case "upsert":            # 节点按 uri 合并 + 关系 MERGE
        labels := schemaRegistry.LabelsFor(ev.entity_type)   # 多 Label（含 EntityType extends 继承）从 Schema 派生，不进 payload
...
# 由 ev.entity_type（topology_events NOT NULL 列）经 SchemaRegistry 解析多 Label（含 extends 继承）。
...
func RebuildDefault(ctx context.Context) error {
    ...
    for offset := 0; ; offset += batchSize {
        batch, err := queryEventsPaged(ctx, snap.AsOfEventTime, time.Now(), offset, batchSize)
        ...
        if err := repo.WithTx(ctx, func(tx pgx.Tx) error {              // ③ 翻 applied（PG 自身事务；与 ② 非原子，靠 MERGE 幂等兜底）
...
形态：`cmd` 子命令 或 MCP 工具；批处理（UNWIND 批量，500/批）。**所有 `applied=true` 写入统一经 `MarkApplied` helper**，与 consumer 共用。`applyBatchToDefault(ctx, graph, batch)` 按 action 调 `graph.Upsert` / `graph.DeleteByURIs` / `graph.DeleteRelations`（增量方法，与步骤 2 载入基线的 `BulkCreate` 区分；与 §4.4 `applyEvent` 仅作用于内存 model 不同）。证明 `default` 非权威、可随时从 PG 再生。
...
CREATE TABLE topology_events (
    ...
    source         TEXT NOT NULL,                   -- webhook|kafka|channel  (不含 fullsync，见 §6.1)
...
| `event.retention_days` | int | 7 | `EVENT_RETENTION_DAYS` | 事件保留窗口 |
...
> `topology_events.source` 枚举为 `webhook|kafka|channel`，**不含 `fullsync`**——FullSync 走 `ClearDB + BulkCreate` 整批替换 default，不产事件行；它通过"建 checkpoint 快照"间接为回放提供基线（§6.4 cutover 例外）。
...
1. cutover 时对 `default` 做一次**基线快照 B0**（吸收全部存量态，记录其 `AsOfEventTime` = cutover 时刻）
2. `Reconstruct` / `RebuildDefault` 统一以「最近快照 + 其后事件」为源，**不假定事件覆盖全部历史**
3. `Reconstruct(T < B0)`：以 B0 为基线重放 (B0, T] 事件；若 T < 最旧保留快照 → `ErrSnapshotEvicted`
...
- 签名：`FlapCount(uri string, field string, window time.Range) int`（`field` 如 `status`/`admin_status`，默认观测字段集合可配）
- 算法：按 `(event_time, id)` 升序取该 uri 在 window 内、针对 `field` 的事件，计数相邻值不相等的翻转次数（`a→b→a` 计 2 次翻转）
- 关系 flap：`delete_relation` 反复增删按 (type,from,to) 计数
...
**Neo4j 健康判定**（`isNeo4jUnavailable`）：连接错误 / 超时 / 驱动关闭 → 降级；Cypher 语法 / 数据错误 → **不降级**（真 bug，降级会掩盖）。
...
| 任务ID | 任务名称 | 工时 | 前置 | 交付物 |
|--------|---------|------|------|--------|
| V3-01 | PG 事件表 Schema + 分区 + 迁移 | 2天 | 无 | `migrations/000002_topology_events.up.sql`（表+首个分区+DEFAULT 分区）+ `000003_snapshots_add_as_of.up.sql`（快照加列）+ **次日分区预建作业** |
| V3-02 | TopologyEventRepository CRUD + 时间窗/uri 查询 + 分页查询 | 1.5天 | V3-01 | `internal/repository/topology_event_repo.go` |
| V3-03 | 保留策略 + 压实作业（与快照 TTL 联动） | 1.5天 | V3-01, V3-02 | **过期分区 DROP** + 联动淘汰逻辑（§6.3 算法）+ `ErrSnapshotEvicted` 哨兵 |

- [ ] V3-01 `migrate up` 建表 + 首个分区；次日预建作业工作；DEFAULT 分区有数据时告警
- [ ] V3-02 按 uri / 时间窗查询正确；分页查询支持 RebuildDefault 批处理
- [ ] V3-03 旧快照淘汰时连带淘汰其之前事件分区（grace 内不删）；T 早于最旧快照返回 `ErrSnapshotEvicted`
...
| 任务ID | 任务名称 | 工时 | 前置 | 交付物 |
|--------|---------|------|------|--------|
| V3-04 | EventSink + Ingest + 扇出 + payload schema + 事务封装 | 2天 | V3-02 | `SyncService.Ingest` + `normalizeToTopologyEvent` 扇出（1→N 行）+ payload JSONB schema 文档化 + `WithTx(ctx, fn)` 跨表事务封装；`events.SyncEvent` 增 `EventIDs []int64`；webhook/kafka 两入口改走 Ingest |
| V3-05 | Outbox + relay + partition-by-uri + 幂等去重 | 2天 | V3-04 | `event_outbox` + relay（唯一投递方，含 §5.1 配置）+ 生产者按 uri 设 Key/HashPartitioner + 批量事件拆单 uri 行 + outbox outbox-to-bus envelope 含 `event_id` |

- [ ] V3-04 两入口都走 Ingest；批量 delete 的 N 个 uri 各落 1 行（RCA 可查）；`WithTx` 包裹 topology_events + event_outbox 同事务
- [ ] V3-05 PG 写成功后 bus 投递失败 → relay 重投成功，事件不丢；同 uri 事件同分区有序；bus envelope 含 `event_id`

### Phase 3: 投影与自愈（V3-06 ~ V3-07）

| 任务ID | 任务名称 | 工时 | 前置 | 交付物 |
|--------|---------|------|------|--------|
| V3-06 | main.go 接 StartConsumer + default 降为物化投影 + MarkApplied | 1天 | V3-05 | 增量链路接通（修复 V2 缺口）；consumer 先 graph 投影、再 `MarkApplied` 翻转 applied（非原子跨库，靠 MERGE 幂等+对账兜底）；MarkApplied 幂等（已 true 行 no-op）；投影积压水位指标 |
| V3-07 | RebuildDefault（自愈/灾备） | 1.5天 | V3-06 | `cmd` 子命令/MCP 工具：`ClearDB(default) + loadSnapshot(最近) + 重放其后事件`（复用 §4.6，500/批，MarkApplied 共用） |

- [ ] V3-06 webhook → PG → bus → default 全链路贯通；`applied` 正确翻转（仅经 MarkApplied helper）
...
| 任务ID | 任务名称 | 工时 | 前置 | 交付物 |
|--------|---------|------|------|--------|
| V3-10 | 周期 FullSync 调度器 | 1.5天 | V3-06 | 后台 ticker（`event.fullsync_interval`，默认 1h，0 禁用）触发 FullSync；与 `GraphLock` 互斥；失败重试 + SLO 告警 |

- [ ] V3-10 每 `fullsync_interval` 自动触发 FullSync 并建 checkpoint；连续失败 N 次告警
...
| Phase 4: 回放+diff | V3-08 | 1 | 3.5 天 |
| Phase 5: 调度器 | V3-10 | 1 | 1.5 天 |
| Phase 6: 验收 | V3-09 | 1 | 1.5 天 |
| **合计** | | **10** | **18 天** |

**风险缓冲**：建议增加 20%，总计 **~22 天** (1人) / **~12 天** (2人并行)。
...
| 3 | 统一接入 + 扇出 | webhook / channel / kafka 三来源；批量 delete N uri | 都落 PG；每个 uri 各 1 行（RCA 可查） |
...
| 双写发散（PG 与 default） | 中 | 高 | Outbox + `applied` 水位 + 周期对账/发散检测 + `RebuildDefault` 自愈（§4.3） |
